...
aoi_cb_message(void *ud, uint32_t watcher, uint32_t marker) {
    ...
    lua_rawgetp(L, LUA_REGISTRYINDEX ,aoi_cb_message);
    lua_pushinteger(L,watcher);
    lua_pushinteger(L,marker);
    lua_pcall(L, 2, 0 , trace);
}
static int
_aoi_create(lua_State *L) {
    struct laoi_space *lspace = malloc(sizeof(*lspace));
...
_aoi_dump(lua_State *L){
    struct laoi_space *lspace = get_lspace(L, 1);
    printf(" count memory = %d, max memory = %d, current memory = %d\n",
     lspace->cookie->count, lspace->cookie->max, lspace->cookie->current);
