...
local server = runconf.server.loginserver
...
function server.auth_handler(args)
	...
	logger.info("auth_handler is performing server=%s token=%s sdkid=%d", server, token, sdkid)
...
function server.login_handler(server, uid, secret)
	...
	-- 登录游戏服务器
	logger.info(string.format("uid=%d is logging to gameserver %s ...", uid, server))
